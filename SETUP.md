...
## Updating an already-running install (this round: multi-select vacancy, simpler buy list)
- **Vacancy: long-press to select multiple rooms**, then update them all at once (e.g. mass move-out after a group leaves). A short tap still updates one room as before.
- **Expected vacate date / duration of stay** — optional, shown only when marking a room Occupied. Fill in either the date or the number of days and the other fills itself in. Rooms show "Vacates in Nd" on the grid, and flag in red if the date's passed.
- **Shopping list simplified** — the "Type" field is gone (sourcing rule now just uses the cost estimate: no estimate defaults to "hardware run"). The room picker now defaults to **General** instead of a blank dash, for shared/non-room-specific items like window cleaner.

To update: same two steps as always — re-run `schema.sql` (adds one new column, safe to re-run), and replace your live `index.html`, pasting your Supabase URL/key back into the two config lines.

## Updating an already-running install (previous round: badges, snapshots, live sync, and more)
This update adds:
